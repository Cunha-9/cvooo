...
xgrid; 
zoom_rect([0, min(rad2deg(u_a(:,2))), 50, max(rad2deg(u_a(:,2)))]);
...
xgrid; 
zoom_rect([0, min(rad2deg(u_b(:,1))), 50, max(rad2deg(phi_ref_vec))]);
...
xcos_file = 'C:\Users\tiago\Desktop\cvoo2\scilab\cvoo2_2_2.zcos';
